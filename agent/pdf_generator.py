...
def generate_pdf_report(
    ...
    prefs_text = report_data.get("investor_preferences", "")
    if prefs_text:
        story.append(Paragraph(f"Investor Preferences: {prefs_text}", styles["body_small"]))
        story.append(Spacer(1, 14))

    # ── SECTION 5: DISCLAIMER ─────────────────────────────────────────────────
    story += _divider(color=RED, thickness=0.3)
    story.append(Paragraph("⚠️ Legal Disclaimer", styles["section_header"]))
